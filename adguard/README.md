...
**A second real incident, found after the fix above was already live**: the household's internet started failing entirely a few minutes after connecting Tailscale, every time, only recoverable by disconnecting Tailscale - not a DNS-specific symptom, `ping` itself stopped working. Root cause: `socat`'s `fork` mode never terminates a forked child that handled a UDP query, since UDP has no "connection closed" signal the way TCP does - each child just sits there indefinitely waiting for more traffic from that same peer. Under real sustained household DNS traffic this grew to 2000+ leaked processes within about an hour, eventually exhausting the relay container and causing new connections (TCP included) to get refused. Fixed by adding `-T5` (a 5-second inactivity timeout) to every `socat` invocation in both `tailscale-adguard/dns-relay` and the structurally-identical `macvlan-host-shim` relay (the latter carries far less traffic so hadn't hit the failure point yet, but had the same latent bug) - verified against the real `adguard` target over the real network path before deploying: a burst of queries left 13 processes alive immediately, down to 3 seven seconds later. If you're writing a `socat ... ,fork` UDP relay of your own, always pair it with `-T<seconds>` - there is no default timeout, and the leak is invisible until it's already taken the service down.

**A third incident, found while verifying the second fix**: `macvlan-host-shim`'s own host-route-repair loop (see above) uses a shell-local `for ip in ...` loop variable - the exact same bare-`$IDENTIFIER`-interpolation bug described above, just pre-existing in this file rather than newly introduced. `$ip` silently collapsed to empty, turning `ip route add "$ip/32" ...` into `ip route add "/32" ...`, which always fails and left the container permanently reporting unhealthy. A leftover route from an earlier one-off manual fix masked the practical impact - the internal-domain conditional-forward kept working anyway, since the route already existed at the kernel level and container recreates don't touch host-level routing state. It would have failed for real on the next NAS reboot, when that manual route would finally be gone. Fixed by escaping `$ip` as `$$ip` in both the startup command and the healthcheck (both had the identical bug). Lesson: after fixing one bare-`$VAR` interpolation bug in a compose file, grep the rest of that file (and any sibling files using the same command-scripting style) for the same pattern rather than assuming it was a one-off.
