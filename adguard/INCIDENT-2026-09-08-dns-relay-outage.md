...
Status as of last edit: **step 2's cutover is DONE, live, and fully
verified** - `dnsdist` replaced `socat` as the actual relay serving real
traffic on `:53` (`tailscale-adguard`'s `dns-relay`), validated first via a
`:5300` canary (0/500 failures at 100x+ the outage-causing query rate - see
[[adguard-dnsdist-burst-test-results-2026-09-08]] for full data). The
container's file-descriptor limit was raised (Docker default 1024 ->
65536), and its healthcheck was fixed through several real bugs (see
below) to a genuinely authenticated check, confirmed live returning the
real `dnsdist 2.0.9` version string, not a false-positive pass. Step 1
(static IP) also done. Steps 3-6 (deleting the `macvlan-host-shim` relay,
`oom_score_adj`, tugtainer TZ, cpuset) **not yet started** - this is the
next real work remaining, not just polish. This file is the durable,
git-tracked record - read this before re-investigating any
AdGuard/Tailscale DNS outage, so the same investigation doesn't have to
happen twice.

## The healthcheck saga - a real dnsdist 2.0.9 bug, worth knowing if reused elsewhere

`dnsdist`'s CLI exits `0` even when its console commands fail entirely -
confirmed multiple times live. This makes an unauthenticated or
wrongly-authenticated healthcheck a silent false positive: it will report
"healthy" regardless of whether `dnsdist` is actually alive. Getting a
*genuinely* authenticated healthcheck working took three iterations:

1. No console key at all -> every connection silently refused, CLI still
   exits 0, empty output. Fixed by adding `console.key` to the YAML config
   and passing `dnsdist -k "$KEY"` in the healthcheck.
2. That still failed - `dnsdist -e ... -k "KEY" -c 127.0.0.1:5199` reports
   `"the currently configured console key is not valid"` **even with the
   exact correct key**, reproduced in a clean isolated test (fresh server,
   a key freshly generated by dnsdist's own `makeKey()`, still rejected).
   **This is a real bug/limitation in dnsdist 2.0.9's `-k`/`--setkey` CLI
   flag when combined with `-e`/`-c` directly on the command line** - not
   a configuration mistake. The docs' *other* documented pattern - a
   client config file containing `setKey(...)` + `controlSocket(...)`,
   loaded via `dnsdist -C <file> -c -e '<cmd>'` - works correctly.
   Switched to that: `dnsdist-config-init` now also writes
   `dnsdist-client.conf`, and the healthcheck uses `-C`/`-c`/`-e` against
   it instead of `-k`.
3. That STILL failed on the next redeploy - genuinely this time (exit code
   1, not the earlier false 0), with a real, useful error:
   `unexpected symbol near '#'`. **`dnsdist-client.conf` is parsed as
   Lua, not YAML** - `#` is not a Lua comment marker (`--` is). Introduced
   by adding explanatory comments to the generated file using the wrong
   syntax; the isolated test that proved the `-C`/`-c` approach works never
   included any comments, so it didn't catch this. Fixed by using `--` in
   that specific generated file only (`dnsdist.yml` stays YAML, `#` is
   correct there).

**If writing a dnsdist client-mode healthcheck (or anything using
`dnsdist -k`) anywhere else**: don't trust `-k` on the command line in
this dnsdist version - use a client config file with `setKey()` +
`controlSocket()` instead, and remember that file is Lua, not YAML, if it
needs comments.
