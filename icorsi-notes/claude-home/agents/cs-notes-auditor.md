...
- **Language consistency.** If a target language was specified (check `_notes.md`'s `## Coverage status`), confirm the notes are written in it throughout - prose, headings, AND template boilerplate (title-page labels, "personal addition" callout label, TOC title). A leftover untranslated boilerplate string from the template default is a `MINOR` finding; mixed-language prose is `MAJOR`.
