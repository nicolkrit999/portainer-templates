...
4. Stay faithful: report only what is actually in the source. Do **not** invent, fill gaps, explain, or add examples - flag gaps instead ("no worked example given for X").
   - Keep definitions/terms in the source's **original language** even if the target notes will be authored in a different one - `cs-notes-author` translates when writing, you extract faithfully.
