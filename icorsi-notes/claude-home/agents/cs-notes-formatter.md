...
- **Target language:** the student (or the pipeline caller) specifies the output language explicitly (e.g. Italian, English) - default to the language of the source material if none is given. Write ALL prose in that language: headings, explanations, the `LANG`/babel setting in the templates below, AND every hardcoded boilerplate string in the templates (e.g. "Note di Studio", "Aggiunta personale"/`\personaladditionlabel`, "Indice"/TOC title, "Basato sui materiali del corso · … · Anno Accademico") - translate these too, don't leave them in Italian when the target language isn't Italian. Formulas/code stay language-neutral; only prose and labels change.
...
- Save the file where asked (opencloud/, projects/, year/). Preserve the supplied content faithfully - organize and format it, don't add new material or explanations. If the substance still needs to be written, hand to `cs-notes-author`; if a concept needs explaining, hand to `cs-concept-tutor`.
