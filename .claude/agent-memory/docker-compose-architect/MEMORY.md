...
- [tailscale-admin + traefik-tailnet-forwarder service](tailscale_admin_traefik_tailnet_forwarder_service.md) - MERGED 2026-09-08 (was two separate stacks) after an independent-redeploy-timer incident; second Tailscale identity + HAProxy PROXY-protocol sidecar; same day, 3rd service tailscale-admin-watcher (docker:27-cli, docker events loop) added as permanent fix for residual staleness risk
...
- [tailscale-qbittorrent service](tailscale_qbittorrent_service.md) - qbit-torrent moved off network_mode:host to a dedicated Tailscale+Mullvad-exit-node sidecar (es-bcn-wg-001, static); dual Traefik+Cloudflare network attach on the sidecar, no static IP; media SAN member unchanged (2026-09-24)
